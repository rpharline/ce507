...
def evaluateElementBernsteinBasisAtParamCoord( uspline, elem_id, param_coord ):
    ...
    new_x = basis.Change_of_Domain(elem_domain,[-1,1], param_coord)
    for n in range( 0, elem_degree + 1 ):
        elem_bernstein_basis[n] = basis.evalBernsteinBasis1D(new_x, elem_degree,elem_domain,n) # Evaluate the Bernstein basis at the parametric coordinate
...
def evaluateElementSplineBasisAtParamCoord( uspline, elem_id, param_coord ):
    ...
    elem_spline_basis = numpy.dot(elem_ext_operator, elem_bernstein_basis) # Apply the extraction operator onto its Bernstein basis at the param coord
